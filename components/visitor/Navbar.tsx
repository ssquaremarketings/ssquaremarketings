  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all ${scrolled ? 'bg-white/95 shadow-sm backdrop-blur' : 'bg-transparent'}`}>
      <div className="container-shell flex h-20 items-center justify-between gap-4">
        ...
        <div
          ref={menuRef}
          className={`absolute left-4 right-4 top-20 z-50 rounded-2xl border border-slate-200 bg-white p-4 shadow-soft transition-all lg:static lg:flex lg:items-center lg:gap-6 lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none ${open ? 'block' : 'hidden lg:flex'}`}
        >
